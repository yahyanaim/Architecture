...
import { 
  Workflow, Briefcase, Layers, FolderTree, ShieldCheck, RefreshCw, 
  CreditCard, KeyRound, CheckCircle2, Zap, Lock, Activity, Sparkles 
} from 'lucide-react';
            ...
            <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-6 text-left max-w-5xl mx-auto animate-in fade-in duration-300">
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  Dual-token security: short-lived 15m access JWTs paired with 30-day hashed rotating refresh tokens. The frontend Axios client intercepts 401s, silently issues a replacement pair via <code className="text-xs bg-gray-100 px-1 py-0.5 rounded">/api/auth/refresh</code>, and replays requests transparently without dropping active users.
                </p>
                <div className="mt-4 pt-4 border-t border-gray-100 flex items-center text-xs text-gray-500 gap-1.5 font-medium">
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  Shared-schema tenancy enforced by <code className="text-xs bg-gray-100 px-1 py-0.5 rounded">resolveTenant</code> middleware and database-hydrated accounts. CI includes an automated tripwire test that parses all repository SQL statements and route definitions, preventing cross-tenant data leaks.
                </p>
                <div className="mt-4 pt-4 border-t border-gray-100 flex items-center text-xs text-gray-500 gap-1.5 font-medium">
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  The <code className="text-xs bg-gray-100 px-1 py-0.5 rounded">authorizeAdmin</code> middleware verifies roles against freshly hydrated database accounts (<code className="text-xs bg-gray-100 px-1 py-0.5 rounded">req.account.role</code>) rather than stateless JWT claims alone, immediately revoking privileged access upon admin demotion.
                </p>
                <div className="mt-4 pt-4 border-t border-gray-100 flex items-center text-xs text-gray-500 gap-1.5 font-medium">
          ...
          {activeTab === 'billing' && (
            <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-6 text-left max-w-5xl mx-auto animate-in fade-in duration-300">
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  Direct REST integration with raw HMAC-SHA256 signature verification and timing-safe checks. Features an idempotent webhook ledger (<code className="text-xs bg-gray-100 px-1 py-0.5 rounded">webhook_events</code>), dunning grace periods (7-day access upon payment failure), and real-time plan synchronization between organizations and subscriptions.
                </p>
                <div className="mt-4 pt-4 border-t border-gray-100 flex items-center text-xs text-purple-600 gap-1.5 font-medium">
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  SQLite-backed background queue with exponential backoff and dead-letter parking (<code className="text-xs bg-gray-100 px-1 py-0.5 rounded">dead</code>). Includes atomic job leases (<code className="text-xs bg-gray-100 px-1 py-0.5 rounded">status = 'running'</code>) to prevent double-execution, plus an automatic zombie recovery reaper for tasks interrupted by worker restarts.
                </p>
                <div className="mt-4 pt-4 border-t border-gray-100 flex items-center text-xs text-blue-600 gap-1.5 font-medium">
                  <CheckCircle2 className="w-4 h-4" />
