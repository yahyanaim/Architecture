...
export function UserList() {
  ...
  if (!filteredUsers || filteredUsers.length === 0) {
    ...
    const isCurrentUserOnly = totalUsers > 0 && currentUser && users?.some(u => u.id === currentUser?.id);
    
    if (isCurrentUserOnly) {
      return (
        <div className="text-gray-500 text-center py-12">
          <p>You're the only user in the system.</p>
          <p className="mt-2">Use the form above to add new users.</p>
        </div>
      );
    }
    ...
    return (
      <div className="text-gray-500 text-center py-12">
        <p>No users found.</p>
        <p className="mt-2">Be the first to create a user using the form above.</p>
      </div>
    );
  }
        ...
        {filteredUsers.map((user) => (
          ...
          >
            <div className="flex justify-between items-start">
              ...
              <span className={`px-2 py-0.5 text-[10px] uppercase tracking-wider font-bold rounded-full ${user.role === 'admin' ? 'bg-black text-white' : 'bg-gray-100 text-gray-800'}`}>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 flex flex-col gap-4">
              <div>
                ...
                <p className="text-base font-medium text-black">{selectedUser?.name || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-1">ID</p>
                <p className="text-base font-medium text-black font-mono">{selectedUser?.id || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-1">Email</p>
                <p className="text-base text-gray-700">{selectedUser?.email || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-1">Status</p>
                <div className="flex items-center gap-2 mt-1">
                  <span className={`w-2.5 h-2.5 rounded-full ${selectedUser?.isActive ? 'bg-black' : 'bg-gray-300'}`} />
              ...
              <div>
                ...
                <span className={`px-2 py-1 text-xs font-bold rounded-full ${selectedUser?.role === 'admin' ? 'bg-black text-white' : 'bg-gray-100 text-gray-800'}`}>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        This will permanently delete <strong>{selectedUser.name}</strong>. This action cannot be undone.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
